...
public class UpdateCartServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        for(CartItem item: cart){
            String paramName = "productQuantity_" + item.getProductId();
            String quantityParam = request.getParameter(paramName);
            if (quantityParam != null) {
                try {
                    int newQuantity = Integer.parseInt(quantityParam);
                    if (newQuantity > 0) {
                        item.setProductQuantity(newQuantity);  // Cập nhật số lượng sản phẩm
                    }
                } catch (NumberFormatException e) {
                    // Xử lý trường hợp số lượng không hợp lệ (có thể là chuỗi không phải số)
                    e.printStackTrace();
                }
            }
